...
## Guided Demo three-act rebuild

### Scope
- Replaced the eight-step evaluator row with `MISSION → ATTACK → SAFE OUTCOME`.
- Rebuilt `/demo` as five focused scenes: mission, safe product, hidden subscription attack, payment boundary, and one receipt-style outcome.
- Added a controlled storefront, canonical Volt Mini 10K pricing, plain-language safety-rule summary, explicit ALLOW/BLOCK explanations, and a compact Security Proof drawer.
- Kept `/demo/shopping` unchanged as the Advanced Security Lab.

### Interaction and safety
- Read-only proposal/evaluation calls are coordinated only after explicit evaluator actions.
- Product addition still requires the explicit `Add Product Safely` Safe Action Gate request.
- Recovery execution still requires the explicit `Continue Without Membership` action.
- BLOCK exposes no override, approval, or execution control.
- `Stop Before Payment` records the safe terminal action and verifies audit without payment, order submission, data sharing, or external navigation.
- Security Proof is read-only and contains the contract summary, timeline, policy findings, recovery summary, audit result, and nested advanced evidence.
